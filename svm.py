...
def local_dev():
    ...
    for c in [.1, .3, .6, 1, 3, 6, 10, 30]:        
        classifier = svm.SVC(C=c).fit(X_train, y_train)
...
def submission():
    ...
    classifier = svm.SVC(C=10).fit(X, y)    # C value derived from local testing
    predictions = classifier.predict(X_test_kaggle)
